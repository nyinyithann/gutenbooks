...
function Bloke({ detail, count }) {
    ...
    return (
      <div className="flex gap-3">
        ...
        <div>
          {names.map((n) => (
            <div key={n}>
              <span className="max-w-[20rem]">{n}</span>
            </div>
          ))}
        </div>
      </div>
  ...
  return (
    <div
      className={`flex flex-col gap-[0.2rem] text-[0.9rem] font-sans w-full py-2 ${
        count > 1 ? 'border-b-[1px] border-b-100 shadow shadow-50' : ''
      }`}
    >
        ...
        {values.map((v) =>
          v.length > minlen ? (
            <div key={v} className="flex min-h-[1.3rem]">
              <span className="text-[0.9rem] font-sans">{v}</span>
            </div>
          ) : null
        )}
      </div>
