...
function AuthorLink({ link, site, title }) {
  return (
    <div className="flex justify-start items-center">
      <span className="flex-shrink-0 w-[4.5rem] text-600 text-left justify-start items-center p-[1px] px-2 m-[2px] bg-50 rounded-r-full">
        {site}
      </span>
      <a
        href={link}
        target="blank"
        className="underline hover:cursor-pointer hover:text-600 text-500 pl-2"
      >
        {title}
      </a>
    </div>
...
function About() {
  return (
    <div className="flex flex-col gap-2 text-600 font-mono text-center h-full items-center justify-center">
      <div className="-mt-40">
        <h1 className="text-2xl text-900 font-bold font-sans">Gutenbooks</h1>
        <div className="pt-4">
          <a
            target="_blank"
            rel="noreferrer"
            className="text-800 underline text-xl text-center inline font-sans"
          ...
          <div className="flex flex-col justify-center items-center font-sans font-normal text-[0.7em] mt-4 pt-2 border-t border-200">
            <div className="w-24 h-24 relative mb-4">
              <img
                ...
                alt="mygithub"
                className="w-full h-full rounded-full border-4 border-300 sepia-0 saturate-150"
              />
